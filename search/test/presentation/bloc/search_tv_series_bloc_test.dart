...
void main() {
  ...
  test('initial state should be empty', () {
    expect(searchBloc.state, SearchTvSeriesEmptyState());
  });

  blocTest<SearchTvSeriesBloc, SearchTvSeriesState>(
      'Should emit [Loading, HasData] when data is gotten successfully',
      build: () {
        when(mockSearchTvSeries.execute(tQuery)).thenAnswer((_) async =>
            Right(tTVSeriesList));
      ...
      [
        ...
        SearchTvSeriesHasDataState(tTVSeriesList)
      ],
      verify:(bloc){
  ...
  blocTest<SearchTvSeriesBloc, SearchTvSeriesState>(
      ...
      [
        SearchTvSeriesLoadingState(),
        SearchTvSeriesErrorState('Server Failure')
      ],
      verify:(bloc){
        verify(mockSearchTvSeries.execute(tQuery));
      }
